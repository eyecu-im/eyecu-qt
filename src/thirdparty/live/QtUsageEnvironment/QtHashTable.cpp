...
// Qt Hash Table implementation
// Implementation

#include "QtHashTable.hh"
#include "strDup.hh"
...
QtHashTable::QtHashTable(int keyType)
...
void* QtHashTable::Add(char const* key, void* value) {
  void* oldValue;
  unsigned index;
...
Boolean QtHashTable::Remove(char const* key) {
  unsigned index;
  TableEntry* entry = lookupKey(key, index);
...
void* QtHashTable::Lookup(char const* key) const {
  unsigned index;
  TableEntry* entry = lookupKey(key, index);
...
unsigned QtHashTable::numEntries() const {
  return fNumEntries;
}

QtHashTable::Iterator::Iterator(QtHashTable const& table)
  : fTable(table), fNextIndex(0), fNextEntry(NULL) {
}

void* QtHashTable::Iterator::next(char const*& key) {
  ...
  QtHashTable::TableEntry* entry = fNextEntry;
...
void QtHashTable::assignKey(TableEntry* entry, char const* key) {
...
void QtHashTable::deleteEntry(unsigned index, TableEntry* entry) {
...
void QtHashTable::deleteKey(TableEntry* entry) {
...
unsigned QtHashTable::hashIndexFromKey(char const* key) const {
  unsigned result = 0;
