...
{
	FMultiChat->abortConnection(QString(),false);
...
// *** <<< eyeCU <<< ***
		else if (action==MUC_URL_NICKNAME)
		{
			...
			if (cursor.atBlockStart())
				suffix.prepend(Options::node(OPV_MUC_NICKNAMESUFFIX).value().toString());
...
bool MultiUserChatWindow::messageDisplay(const Message &AMessage, int ADirection)
{
	...
	else if (ADirection == IMessageProcessor::DirectionIn)
	{
		...
		{
			if (!showMultiChatStatusCodes(FMultiChat->statusCodes(AMessage.stanza()),QString(),AMessage.body()) && !isEmptyBody)
...
void MultiUserChatWindow::toolTipsForUser(IMultiUser *AUser, QMap<int,QString> &AToolTips)
{
	...
	{
		QString avatar = FAvatars!=NULL ? FAvatars->avatarHash(AUser->userJid(),true) : QString();
		...
		AToolTips.insert(MUTTO_MULTIUSERCHAT_NICKNAME,QString("<big><b>%1</b></big>").arg(HTML_ESCAPE(AUser->nick())));

		if (AUser->realJid().isValid())
			AToolTips.insert(MUTTO_MULTIUSERCHAT_REALJID,tr("<b>Jabber ID:</b> %1").arg(HTML_ESCAPE(AUser->realJid().uBare())));
		...
		if (!role.isEmpty())
		{
			...
			AToolTips.insert(MUTTO_MULTIUSERCHAT_ROLE,tr("<b>Role:</b> %1").arg(HTML_ESCAPE(roleName)));
		}
		...
		if (!affiliation.isEmpty())
		{
			...
			if (affiliation == MUC_AFFIL_NONE)
				...
				affilName = tr("Owner");
			AToolTips.insert(MUTTO_MULTIUSERCHAT_AFFILIATION,tr("<b>Affiliation:</b> %1").arg(HTML_ESCAPE(affilName)));
		}

		QString ttStatus;
		QString statusText = AUser->presence().status;
		QString statusName = FStatusChanger!=NULL ? FStatusChanger->nameByShow(AUser->presence().show) : QString();
...
void MultiUserChatWindow::insertUserMention(IMultiUser *AUser, bool ASetFocus) const
{
	if (AUser && FEditWidget && AUser!=FMultiChat->mainUser())
	{
		if (ASetFocus)
			FEditWidget->textEdit()->setFocus();
		QString sufix = FEditWidget->textEdit()->textCursor().atBlockStart() ? Options::node(OPV_MUC_NICKNAMESUFFIX).value().toString().trimmed() : QString();
...
bool MultiUserChatWindow::showMultiChatStatusCodes(const QList<int> &ACodes, const QString &ANick, const QString &AMessage)
{
	if (!ACodes.isEmpty())
	{
		...
		if (ACodes.contains(MUC_SC_SELF_PRESENCE))
			statuses.append(qMakePair<QString,int>(QString(),0));
...
void MultiUserChatWindow::showMultiChatUserMessage(const Message &AMessage, const QString &ANick)
{
	if (FMessageStyleManager)
	{
		...
		options.senderColor = FViewWidget->messageStyle()!=NULL ? FViewWidget->messageStyle()->senderColorById(ANick) : QString();
...
void MultiUserChatWindow::updatePrivateChatWindow(IMessageChatWindow *AWindow)
{
	...
	if (user)
	{
		...
		AWindow->updateWindow(tabIcon,name,tr("%1 - Private Chat").arg(name),QString());
	}
}
...
void MultiUserChatWindow::onMultiChatUserChanged(IMultiUser *AUser, int AData, const QVariant &ABefore)
{
	if (AData == MUDR_PRESENCE)
	{
		...
		{
			QString show = FStatusChanger ? FStatusChanger->nameByShow(presence.show) : QString();
			if (!FUsers.contains(AUser))
			{
...
void MultiUserChatWindow::onMultiChatUserKicked(const QString &ANick, const QString &AReason, const QString &AByUser)
{
	...
	showMultiChatStatusMessage(tr("User %1 has been kicked from the conference%2 %3")
		...
		.arg(!AByUser.isEmpty() ? tr(" by moderator %1").arg(AByUser) : QString())
...
void MultiUserChatWindow::onMultiChatRoomDestroyed(const QString &AId, const QString &AReason)
{
	if (AId == FDestroyRequestId)
	{
		...
		QString html = tr("This conference was destroyed by owner %1 %2")
			.arg(!AReason.isEmpty() ? HTML_ESCAPE("("+AReason+")") : QString())
			.arg(QString("<a href='%1'>%2</a>").arg(exitUrl.toString(),tr("exit")));
...
void MultiUserChatWindow::onMultiChatEditWidgetKeyEvent(QKeyEvent *AKeyEvent, bool &AHooked)
{
	if (FMultiChat->isOpen() && AKeyEvent->modifiers()+AKeyEvent->key() == NICK_MENU_KEY)
	{
		...
		QString suffix = cursor.atBlockStart() ? Options::node(OPV_MUC_NICKNAMESUFFIX).value().toString().trimmed() : QString();
		if (FCompleteNicks.count() > 1)
		{
			...
			else
			{
				...
				cursor.beginEditBlock(); // *** <<< eyeCU >>> ***
				cursor.insertText(*FCompleteIt + suffix + " ");
				cursor.endEditBlock(); // *** <<< eyeCU >>> ***
...
// *** >>> eyeCU >>> ***
	...
	else if (action == FDestroyRoom)
	{
		...
		{
			bool ok = false;
			QString reason = QInputDialog::getText(this,tr("Destroy Conference"),tr("Enter a message:"),QLineEdit::Normal,QString(),&ok);
			if (ok)
...
void MultiUserChatWindow::onChangeUserRoleActionTriggeted(bool)
{
	...
	if (action)
	{
		...
		if (role == MUC_ROLE_NONE)
			reason = QInputDialog::getText(this,tr("Kick User - %1").arg(nick),tr("Enter a message:"),QLineEdit::Normal,QString(),&ok);
...
void MultiUserChatWindow::onChangeUserAffiliationActionTriggered(bool)
{
	...
	if (action)
	{
		...
		if (affiliation == MUC_AFFIL_OUTCAST)
			reason = QInputDialog::getText(this,tr("Ban User - %1").arg(nick),tr("Enter a message:"),QLineEdit::Normal,QString(),&ok);
