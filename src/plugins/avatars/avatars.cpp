...
AdvancedDelegateItem Avatars::rosterLabel(int AOrder, quint32 ALabelId, const IRosterIndex *AIndex) const
{
	Q_UNUSED(AIndex);
/*** <<< eyeCU <<< ***/
	if (AOrder==RLHO_AVATARS && (ALabelId==FAvatarRightLabelId || ALabelId==FAvatarLeftLabelId))
...
bool Avatars::setAvatar(const Jid &AStreamJid, const QByteArray &AData)
{
	...
	if (AData.isEmpty() || !format.isEmpty())
	{
		...
		if (vcard)
		{
			...
			if (FVCardManager->publishVCard(AStreamJid,AStreamJid,vcard))
			{
				published = true;
...
{
/*** <<< eyeCU <<< ***/
	if ((ANode.path() == OPV_ROSTER_VIEWMODE) && !Options::node(OPV_COMMON_ADVANCED).value().toBool())
/*** >>> eyeCU >>> ***/
	{
