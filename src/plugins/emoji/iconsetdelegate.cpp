...
IconsetDelegate::IconsetDelegate(IEmoji *AEmoji, QObject *AParent):
...
{
	QString name = AIndex.data(IDR_NAME).toString();
	if (name.isEmpty())
	...
	else
	{
		...
		if (!AIndex.data(IDR_HIDE_SET_NAME).toBool())
		{
		...
		for (QMap<uint, QString>::ConstIterator it = emojiMap.constBegin();
			 it!=emojiMap.constEnd() && drawRect.bottom()>top && drawRect.right()>left && row<maxRows;
			 ++it)
		{
			QIcon icon = FEmoji->getIconForSet(name, *it, QSize(16,16));
			if (!icon.isNull())
			{
...
QSize IconsetDelegate::sizeHint(const QStyleOptionViewItem &AOption, const QModelIndex &AIndex) const
{
	...
	else
	{
		...
		int iconWidth = qMin(indexOption.rect.width(),emojiMap.count()*(indexOption.decorationSize.width()+space));
